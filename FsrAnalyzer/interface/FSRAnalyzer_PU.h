#ifndef FSRAnalyzer_PU_h_
#define FSRAnalyzer_PU_h_
...
extern bool orderPartByPt(const HepMC::GenParticle* p1, const HepMC::GenParticle* p2);
...
class FSRAnalyzer_PU : public edm::EDAnalyzer {
 public:
  explicit FSRAnalyzer_PU(const edm::ParameterSet&);
  ~FSRAnalyzer_PU();
...
//
FSRAnalyzer_PU::FSRAnalyzer_PU(const edm::ParameterSet& iConfig)
{
  ...
  h_nvtx = fs->make<TH1F>("h_nvtx","# of generated vertices; # of vtx",50,0.,50.);
  ...
  // --- histograms
  for (unsigned int ih=0; ih<n_dr_bins; ++ih) {
    ...
    hname  = Form("h_dr_et_ph_PU_%d",ih);
    htitle = Form("photons from PU in #DeltaR<%4.2f;E_{T} [GeV];#DeltaR",dR_cut);
    h_dr_et_ph_PU[ih] = fs->make<TH2F>(hname.Data(), htitle.Data(),1000,0.,10.,1000,0.,1.);
    ...
    hname  = Form("h_dr_e_ph_PU_%d",ih);
    htitle = Form("photons from PU in #DeltaR<%4.2f;E_{T} [GeV];#DeltaR",dR_cut);
    h_dr_e_ph_PU[ih] = fs->make<TH2F>(hname.Data(), htitle.Data(),1000,0.,10.,1000,0.,1.);
  ...
  h_FSR_frac_p = fs->make<TProfile>("h_FSR_frac_p", "fraction of FSR energy in a #DeltaR cone;#DeltaR", n_dr_bins,0.,1.,0.,1.1);
  h_FSR_frac_p->Sumw2();

  h_frac_fsr_p = fs->make<TProfile>("h_frac_fsr_p","fraction of FSR #gamma energy in a #DeltaR cone;#DeltaR", n_dr_bins,0.,1.,0.,1.1);
  h_frac_fsr_p->Sumw2();

  h_frac_other_p = fs->make<TProfile>("h_frac_other_p","fraction of other #gamma energy in a #DeltaR cone;#DeltaR", n_dr_bins,0.,1.,0.,1.1);
  h_frac_other_p->Sumw2();
   
  h_frac_PU_p = fs->make<TProfile>("h_frac_PU_p","fraction of PU #gamma energy in a #DeltaR cone;#DeltaR", n_dr_bins,0.,1.,0.,1.1);
  h_frac_PU_p->Sumw2();

  h_FSR_totet_p = fs->make<TProfile>("h_FSR_totet_p", "tot E_{T} of FSR energy in a #DeltaR cone;#DeltaR", n_dr_bins,0.,1.,0.,1.1);
  h_FSR_totet_p->Sumw2();

  h_totet_fsr_p = fs->make<TProfile>("h_totet_fsr_p","tot E_{T} of FSR #gamma's in a #DeltaR cone;#DeltaR", n_dr_bins,0.,1.,0.,1.1);
  h_totet_fsr_p->Sumw2();

  h_totet_other_p = fs->make<TProfile>("h_totet_other_p","tot E_{T} of other #gamma's in a #DeltaR cone;#DeltaR", n_dr_bins,0.,1.,0.,1.1);
  h_totet_other_p->Sumw2();
   
  h_totet_PU_p = fs->make<TProfile>("h_totet_PU_p","tot E_{T} of PU #gamma's in a #DeltaR cone;#DeltaR", n_dr_bins,0.,1.,0.,1.1);
  h_totet_PU_p->Sumw2();
